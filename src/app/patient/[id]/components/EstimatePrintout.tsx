...
export default function EstimatePrintout({ data, onBack }: EstimatePrintoutProps) {
    const { toast } = useToast();
    const { patient, costs, images, dentalData, additionalTreatments, selectedPackages } = data;

    const generalImages = useMemo(() => {
        return images?.filter(img => img.category === 'general') || [];
    }, [images]);
    ...
    const handlePrint = () => {
        ...
        const filename = `${patient.name}_예상비용안내서_${surgeryDate}.pdf`;
        const printContent = document.getElementById('estimate-print-area')?.innerHTML;
    ...
    const renderQuadrantTable = (quadrantIds: string[], quadrantTitle: string) => {
        ...
        const allTeethInQuadrant = quadrantIds.flatMap(id => {
            const permanentTooth = dentalData[id];
            const deciduousId = String(parseInt(id, 10) + 400);
            const deciduousTooth = dentalData[deciduousId];
    
            const entries: any[] = [];
    
            // 영구치 정보 처리
            if (permanentTooth && (permanentTooth.status?.length > 0 || permanentTooth.procedures?.length > 0)) {
                entries.push(permanentTooth);
            }
    
            // 잔존 유치 정보 처리 (영구치와 연관)
            if (permanentTooth && permanentTooth.status?.includes('RD') && deciduousTooth && deciduousTooth.procedures?.length > 0) {
                 // 유치 정보를 영구치와 함께 표시하기 위해 영구치 ID를 사용하되, 내용은 유치 정보를 사용
                entries.push({
                    ...deciduousTooth,
                    displayId: deciduousTooth.id, // 실제 유치 번호
                    mainToothId: id // 연관된 영구치 번호
                });
            }
        ...
        if (allTeethInQuadrant.length === 0) return null;

        const groupedTeeth: { [key: string]: any[] } = { incisors: [], canines: [], premolars: [], molars: [] };
        
        allTeethInQuadrant.forEach(tooth => {
            const group = getToothGroup(tooth.mainToothId || tooth.id);
            if (group && groupedTeeth[group] && !groupedTeeth[group].find(t => t.id === tooth.id)) {
                    ...
                    {Object.entries(groupedTeeth).map(([group, teeth]) => {
                       ...
                       return (
                           <React.Fragment key={group}>
                               ...
                               {teeth.map(tooth => {
                                   const displayId = tooth.displayId || tooth.id;
                                   const statusArray = Array.isArray(tooth.status) ? tooth.status.filter(s => s !== 'RD' && s !== 'DT') : [];
                                   ...
                                   return (
                                       <tr key={tooth.id} className={groupColors[getToothGroup(tooth.id)]}>
                                           <td className="font-mono font-medium">{displayId}</td><td className="status-text">{getStatusDisplayString(statusArray)}</td>
                ...
                <div className="max-w-4xl mx-auto my-8 p-4 sm:p-8 bg-white shadow-2xl rounded-lg" data-print-content>
                    ...
                    <div id="estimate-print-area">
                        <div className="header">
                            ...
                            <h1 className="mt-4">예상 비용 안내서</h1>
                        ...
                        <div className="section"><h2 className="section-title">치과 차트 요약 (예상 시술)</h2>
                        ...
                        <div className="section"><h2 className="section-title">환자 사진</h2>
                            {generalImages.length > 0 ? (<div className="image-gallery">{generalImages.map(image => (<div key={image.id} className="image-container"><Image src={image.imageUrl} alt="환자 일반 사진" width={200} height={200} style={{width: "100%", height: "auto"}} /></div>))}</div>) : (<p>업로드된 일반 사진이 없습니다.</p>)}
                        </div>
                        ...
                        <div className="section"><h2 className="section-title">비용 정보</h2>
                            <div className="space-y-6">
                                <table className="cost-summary-table">
                                    <tbody>
                                        ...
                                        <tr className="total-cost-row"><td className='text-right'>총 예상 비용:</td><td>{totalCost.toLocaleString()} 원</td></tr>
                                    </tbody>
                                </table>
                        ...
                        <div className="footer-note"><p><strong>안내 말씀</strong></p><p>본 예상 비용 안내서는 현재까지의 환자 사진과 구강 상태에 대한 시각적 평가를 바탕으로 산출된 개략적인 금액입니다. 수술 당일, 마취 후 진행되는 정밀 구강 검사 및 방사선 촬영 결과에 따라, 숨어있는 추가적인 문제(치근단 농양, 치아 흡수성 병변, 치주 질환 등)가 발견될 수 있습니다.</p><p>이러한 경우, 아이의 통증을 없애고 삶의 질을 높이기 위해 꼭 필요한 추가적인 치료가 진행될 수 있으며, 이에 따라 최종 비용에는 변동이 발생할 수 있음을 미리 안내해 드립니다. 저희는 항상 보호자님과 충분히 소통하며 최선의 치료를 약속드립니다.</p></div>
